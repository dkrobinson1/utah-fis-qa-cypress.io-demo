...
import moviesEntityPage from './pageobjects/moviesEntityPage';
...
Cypress.Commands.add('deleteTestMovie', (movieTitle) => {
    cy.navToMoviesEntityPage();
    MoviesEntityPage.sortByIDLnk().click();
    MoviesEntityPage.getMovieTitleDataFromTable(1).should('contain.text', movieTitle);
    //Delete Added Movie
    MoviesEntityPage.deleteMovieBtn(1).click();
    MoviesEntityPage.deleteMovieConfirmDeleteBtn().click();
    cy.get('.Toastify__toast').should('contain.text', 'deleted');
    cy.log('DELETED TEST MOVIE****: ' + movieTitle);
});
...
Cypress.Commands.add('navToMovieSearchPage', () => {
    HomePage.getMovieMenu().click();
});


Cypress.Commands.add('pickTestMovie', () => {
    cy.wrap(testMovie)
})
...
Cypress.Commands.add('navToMoviesEntityPage', () => {
    HomePage.getAccountNavMnuEntities().click();
...
Cypress.Commands.add('navToMovMnu', () => {
    HomePage.getMoviesMenu.click();
});
